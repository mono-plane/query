...
public class NodeWriteAttributeHandler extends RestartParentWriteAttributeHandler {
    ...
    protected void recreateParentService(OperationContext context, PathAddress parentAddress, ModelNode parentModel, ServiceVerificationHandler verificationHandler) throws OperationFailedException {
        NodeAdd.installRuntimeServices(context, parentAddress, parentModel, verificationHandler, new ArrayList<ServiceController<?>>());
    }

    @Override
    protected ServiceName getParentServiceName(PathAddress parentAddress) {
        return NodeAdd.SERVICE_NAME.append(parentAddress.getLastElement().getValue());
    }
